...
@section('show-content')
    <section class="mb-8">
        <img src="{{$author->url ?? asset('/images/user.png') }}" alt="{{ $author->nickname }}" class="items-center rounded-[5%] max-h-[50%] max-w-[50%] mx-auto mb-2">
        <h1 class="text-center font-bold">{{ $author->nickname }}</h1>
        <h2>Songs:</h2>
        <ul>
        ...
        <div class="btn-right">
            ...
            <form action="{{route('authors.destroy', $author->id)}}" method="post" class="inline">
                @csrf
                @method('DELETE')
                <button class="btn-white px-[15px] py-[10px] hover:px-[16px] m-1">
